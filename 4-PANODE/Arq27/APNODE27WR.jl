...
# include("C:/Users/Propietario/OneDrive/Escritorio/ib/Tesis_V1/MLonNMR/3-SeñalesPANODE/UtilidadesSeñales.jl")

include("/home/juan.morales/PANODE/Parameters.jl")
# include("C:/Users/Propietario/OneDrive/Escritorio/ib/Tesis_V1/MLonNMR/3-SeñalesPANODE/Parameters.jl")
...
# if isfile("C:/Users/Propietario/OneDrive/Escritorio/ib/Tesis_V1/MLonNMR/3-SeñalesPANODE/Arq27/027_Parameters.csv")
#     theta = CSV.read("C:/Users/Propietario/OneDrive/Escritorio/ib/Tesis_V1/MLonNMR/3-SeñalesPANODE/Arq27/027_Parameters.csv", DataFrame)
#     p_nn = Float32.(theta[:,1])
# else
#     println("No se encontraron los parámetros de la red neuronal")
# end
...
function SolveANODE(u0, extra, time_batch)
    ...
    solution = @views Array(solve(prob, Tsit5(), p = p_nn, saveat = time_batch, dtmin = 1e-9))[1, :]
...
# Regularización de la magnitud de la magnitud de S(t) y a(t)
function S_regularization(S_pred)
    loss_S_magnitude = 0
    for i in 1:size(S_pred)[1]
        loss_S_magnitude += sum(abs.(S_pred[i,:]))
    end
    return loss_S_magnitude
end

# function a_regularization(a_pred)
#     loss_a_magnitude = 0
#     for i in 1:size(a_pred)[1]
#         loss_a_magnitude += sum(abs.(a_pred[i,:]))
#     end
#     return loss_a_magnitude 
# end

# Regularización de la magnitud de las derivadas de S(t) y a(t)

function Sprime_regularization(S_pred, time_batch)
    loss_S_smoothness = 0
    for i in 1:size(S_pred)[1]
        loss_S_smoothness += sum(abs.(diff(S_pred[i, :]) ./ diff(time_batch)))
    end
    return loss_S_smoothness
end

# function aprime_regularization(a_pred, time_batch)
#     loss_a_smoothness = 0
#     for i in 1:size(a_pred)[1]
#         loss_a_smoothness += sum(abs.(diff(a_pred[i, :]) ./ diff(time_batch)))
#     end
#     return loss_a_smoothness
# end


function loss_node(batch, time_batch)
    y = PredictSignals(U0, extra_input, time_batch)
    return Flux.mse(y, batch) + λ_S * S_regularization(y)  + λ_S′ * Sprime_regularization(y, time_batch)
end
...
CSV.write("/home/juan.morales/PANODE/Arq27/0$(actual_id)_ParametersWR.csv", df_parameters)
...
if isfile("/home/juan.morales/PANODE/Arq27/0$(actual_id)_losses.csv")
   df_losses = CSV.read("/home/juan.morales/PANODE/Arq27/0$(actual_id)_losses.csv", DataFrame)
   loss = df_losses[:,1]
...
CSV.write("/home/juan.morales/PANODE/Arq27/0$(actual_id)_lossesWR.csv", df_losses)